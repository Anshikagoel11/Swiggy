export default function RestInfo({ restData }) {
 ...
 <span className="text-gray-600 ml-1 font-normal">
        ...
        <p className="text-gray-500 text-sm">{restData?.description}</p>
      </div>
      ...
      <div className="w-[30%] relative">
        ...
        {restData?.itemAttribute?.vegClassifier === "NONVEG" && (
         <div className="bg-white absolute top-1 right-1 p-1 rounded"> <p className=" text-xs text-red-600 font-bold">Non-Veg</p></div>
        )}
        {restData?.itemAttribute?.vegClassifier === "VEG" && (
         <div className="bg-white absolute top-1 right-1 p-1 rounded"> <p className=" text-xs  text-green-600 font-bold">Veg</p></div>
        )}
      </div>
    </div>
  );
